<template>
	<view class="YmContent" :animation="animationData">
			<view class="voucher_center__1">
				<text decode="true" class="voucher_center__47">充值时间,每小时10元</text>
				<view class="voucher_center__48">
					<input :value="money" placeholder="0" @input="money_input" class="money" />
					<text decode="true" class="voucher_center__49">小时</text>
				</view>

				<button v-on:click="recharge()" class="btSubmit">确认支付
				</button>
				<text decode="true"  class="voucher_center__47">当前用户余额{{yue}}</text>
			</view>
	...
	export default {
		...
		methods:{
			...
			money_input(event){
				...
				this.yue=this.usermoney-(this.money*10)
			...
			getusermoney(){
				...
				uni.request({
					...
					success: (res) => {
						console.log(res);
						vue.usermoney=res.data.data.userBalance
						vue.yue=res.data.data.userBalance;
					}
				})
			},
			appLoginWx(){
