<template>
	...
	<view class="r_detail" v-else>
		...
		<view class="total">{{order_info.pay_statement==0?'待支付':'支付'}}：¥ {{allPrice}}</view>
		<view class="mess">
			<view class="mess_01">订单信息</view>
			<view class="mess_02">
				<view class="mess_02_1">
					<view class="mess_02_l">订单编号:</view>
					<view class="mess_02_r">{{orderinfo.consumeOrderId}}</view>
				</view>
				<view class="mess_02_1">
					<view class="mess_02_l">{{order_info.pay_statement==0?'创建时间':'支付时间'}}: </view>
					<view class="mess_02_r">{{orderinfo.consumeCreatetime}}</view>
				</view>
				<view class="mess_02_1">
...
<script>
	...
	export default {
		...
		onLoad(items) {
			...
			console.log("iteminfosss", vue.iteminfo)
			console.log("userid", vue.$store.state.userid)
				vue.init();
			uni.request({
				method: 'POST',
				url: 'http://localhost:8080/Itemconsume/tips',
				success: (res) => {
					console.log(res)
				}
			})
		...
		methods: {
			...
			init(){
				let vue=this;
				vue.iteminfo.itemlist.forEach((item) => {
					// console.log("ID",item.itemId);
					// console.log("名字",item.itemName);
					// console.log("数量",item.count);
					vue.allPrice = vue.allPrice * 1 + vue.accMul(item.count, item.itemPrice) * 1
					console.log("item", item);
				
					const goods = {
						itemId: item.itemId,
						consumeUserid: vue.$store.state.userid,
						itemName: item.itemName,
						consumeCount: item.count,
						consumeOrderId: vue.iteminfo.orderid,
						consumePrice: item.itemPrice
					}
					console.log("vue.goods", goods)
						...
						success: (res) => {
							...
							uni.request({
								...
								success: (response) => {
									...
									Vue.set(res.data.data, "itemImg", response.data.data
										.itemImg)
									vue.orderlist.push(res.data.data);
									console.log("orderlist", vue.orderlist)
								}
							})
						}
					})
